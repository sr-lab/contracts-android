...
/**
 * Screen for constraints that use older, comments-style annotations. Do a simple text search
 * instead of building a much more expensive full-fledged extractor. The extractor will only be
 * build if this script reveals that there are constraints of this type in the dataset analysed.
 *
...
public class ScreenForConstraintsByCommentPattern {
  ...
  public static void main(String[] args) throws Exception {
            ...
            try (InputStream in = zip.getInputStream(e)) {
              ...
              for (Pattern pattern : PATTERNS) {
                if (!foundPatternsInProject.contains(pattern) && pattern.matcher(src).matches()) {
                  LOGGER.info(
                    "Pattern " + pattern.pattern() + " is used in " + f.getAbsolutePath());
    ...
    for (Map.Entry<Pattern, Set<String>> e : usage.entrySet()) {
      LOGGER.info(
        "\t" + e.getKey().pattern() + " is used by " + e.getValue().size() + " program(s) ");
    }
  }
