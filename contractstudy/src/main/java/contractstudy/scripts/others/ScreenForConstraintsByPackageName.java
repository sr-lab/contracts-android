package contractstudy.scripts.others;
...
/**
 * Screen for constraints (or rather constraint classes incl annotations). Screen for constraints
 * that use older, comments-style annotations. Do a simple text search instead of building a much
 * more expensive full-fledged extractor. The extractor will only be build if this script reveals
 * that there are constraints of this type in the dataset analysed. This will walk through a data
 * set (zipped java sources) and simply look for references to package names for specific contract
 * APIs incl annotations in source code. A summary of findings will be printed to the console. The
 * package names are defined in a static field.
 *
...
public class ScreenForConstraintsByPackageName {
  ...
  static String[] PACKAGES = {
    ...
    // http://oval.sourceforge.net/userguide.html#declaring-field-constraints
    ...
    "net.sourceforge.c4j",
    "jass.modern",
    // http://csd.informatik.uni-oldenburg.de/~jass/doc/index.html  older versions use comment-based annotations !
            ...
            try (InputStream in = zip.getInputStream(e)) {
              ...
              for (String pck : PACKAGES) {
                if (!foundPackagesInProject.contains(pck) && src.contains(pck)) {
                  LOGGER.info(
                    "Constraint system " + pck + " is used in " + f.getAbsolutePath() + " cu "
                      + e.getName());
