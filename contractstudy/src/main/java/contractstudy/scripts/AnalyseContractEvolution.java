...
public class AnalyseContractEvolution implements Experiment {
    ...
    return new HashMap<>() {
      {
        ...
        put(DiffResult.CANNOT_BE_CLASSIFIED,
          new File(RESULTS_FOLDER, "contracts_cannot_be_classified.log"));
      }
    };
  }

  private static void resetLogsByDeletingFiles(Map<DiffResult, File> DETAIL_LOGS) {
    for (File log : DETAIL_LOGS.values()) {
      log.delete();
    }
  }

  public static Map<DiffResult, Integer> compareEvolutionAndLogResults(
    Map<DiffResult, File> DETAIL_LOGS) throws Exception {
  ...
  private static void outputEachEvolutionStatsToFile(DiffRecord record, DiffResult result,
    File log) {
