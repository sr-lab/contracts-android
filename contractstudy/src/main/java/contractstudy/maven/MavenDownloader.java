...
public class MavenDownloader {
   ...
   * @return all versions
   */
  public static List<MavenProjectVersion> findVersions(final MavenProjectVersion project)
    throws IOException {
    ...
    while (true) {
      String query = "q=g:\"" + project.getGroupId() + "\"+AND+a:\"" + project.getArtefactId()
        + "\"&core=gav&start=" + start;
      String link = MAVEN_SEARCH_API + "?" + query;

      JSONObject json = new JSONObject(IOUtils.toString(new URL(link), StandardCharsets.UTF_8));
   ...
   */
  public List<MavenProjectVersion> downloadProject(final MavenProjectVersion project)
    throws IOException {
  ...
  public void downloadVersion(final MavenProjectVersion version) throws IOException {
    ...
    String path = groupPath + "/" + version.getArtefactId() + "/" + version.getVersion() + "/"
      + version.getArtefactId() + "-" + version.getVersion() + "-sources.jar";
    ...
    // download POM file
    String pomPath = groupPath + "/" + version.getArtefactId() + "/" + version.getVersion() + "/"
      + version.getArtefactId() + "-" + version.getVersion() + ".pom";
