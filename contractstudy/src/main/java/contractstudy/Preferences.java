...
public class Preferences {
  ...
  static {
    ...
    try (FileInputStream in = new FileInputStream(PREF_FILE)) {
      ...
      LOGGER.warn(
        "Create this as a standard property file with lines key=value, note that this is in .hgignore");
    }
  }
  ...
  private static String getStringPref(String key, String defaultValue) {
    ...
    if (value == null) {
      LOGGER.warn(
        "Cannout find value for key " + key + " in " + PREF_FILE + ", will use default value "
          + defaultValue);
      return defaultValue;
    } else {
      return value;
